...
#ifdef VGA_TEST
#define VGATESTFILE "/tmp/vgaoutfile"
static Bit8u vgt_screen[SCALER_MAXWIDTH * SCALER_MAXHEIGHT * 4];
static FILE* vgt_file = NULL;
static Bitu vgt_index = 0;
static void VGAPRX_SetSize(Bitu width,Bitu height,Bitu bpp,float fps,double ratio,bool dblw,bool dblh);
static void VGAPRX_DrawLine(const void *src);
static bool VGAPRX_StartUpdate(void);
static void VGAPRX_EndUpdate(bool abort);
#endif
...
static void VGA_DrawSingleLine(Bitu /*blah*/) {
	if (GCC_UNLIKELY(vga.attr.disabled)) {
		...
		VGAPRX_DrawLine(TempLine);
	} else {
		Bit8u * data=VGA_DrawLine( vga.draw.address, vga.draw.address_line );	
		VGAPRX_DrawLine(data);
	}
...
static void VGA_VerticalTimer(Bitu /*val*/) {
	...
	if (vga.draw.vga_override || !VGAPRX_StartUpdate())
		return;
...
//	LOG_MSG("ht %d vt %d ratio %f", htotal, vtotal, aspect_ratio );
	...
	if ((width != vga.draw.width) ||
		...
		if (!vga.draw.vga_override) 
			VGAPRX_SetSize(width, height, bpp, (float)fps, aspect_ratio,
...
void VGA_KillDrawing(void) {
	...
	vga.draw.lines_done = ~0;
	if (!vga.draw.vga_override) VGAPRX_EndUpdate(true);
}
...
static void VGAPRX_SetSize(Bitu width,Bitu height,Bitu bpp,float fps,double ratio,bool dblw,bool dblh)
{
	if (!vgt_file) vgt_file = fopen(VGATESTFILE,"w+");
	fprintf(stderr,"\n[VGAProxy]: SetSize(): w=%d; h=%d; bpp=%d; fps=%0.4f; ratio=%0.4f; dblw=%hu; dblh=%hu\n\n",width,height,bpp,fps,ratio,dblw,dblh);
	RENDER_SetSize(width,height,bpp,fps,ratio,dblw,dblh);
}

static void VGAPRX_DrawLine(const void *src)
{
	Bitu s = vga.draw.width * ((vga.draw.bpp+1)/8);
	memcpy(&vgt_screen[vgt_index*s],src,s);
	vgt_index++;
	RENDER_DrawLine(src);
}
...
{
	vgt_index = 0;
	memset(vgt_screen,0,sizeof(vgt_screen));
	return (RENDER_StartUpdate());
}

static void VGAPRX_EndUpdate(bool abort)
{
	if (abort) fprintf(stderr,"[VGAProxy]: EndUpdate(): abort=true\n");
	Bitu pitch = vga.draw.width * ((vga.draw.bpp+1)/8);
	if (vgt_file) {
		Bit32u i,j,s,bt;
		fseek(vgt_file,0,SEEK_SET);
		int ntW = vga.draw.width;
		int ntH = vga.draw.height;
		fwrite(&ntW,sizeof(ntW),1,vgt_file);
		fwrite(&ntH,sizeof(ntH),1,vgt_file);
		for (i=0; i<ntH; i++) {
			for (j=0; j<ntW; j++) {
				Bit8u* px8 = ((Bit8u*)vgt_screen) + i*pitch;
				switch (vga.draw.bpp) {
				case 8:
					s = px8[j];
					bt = render.pal.rgb[s].blue;
					bt |=render.pal.rgb[s].green << 8;
					bt |= render.pal.rgb[s].red << 16;
					break;
				case 15:
					s = ((Bit16u*)px8)[j];
					bt = ((s & 0x001f) * 0x21) >> 2;
					bt |= ((s & 0x03e0) * 0x21) >> 7 << 8;
					bt |= ((s & 0x7c00) * 0x21) >> 12 << 16;
					break;
				case 16:
					s = ((Bit16u*)px8)[j];
					bt = ((s & 0x001f) * 0x21) >> 2;
					bt |= ((s & 0x07e0) * 0x41) >> 9 << 8;
					bt |= ((s & 0xf800) * 0x21) >> 13 << 16;
					break;
				case 32:
					bt = ((Bit32u*)px8)[j];
					break;
				}
				fwrite(&bt,4,1,vgt_file);
			}
		}
	}
	RENDER_EndUpdate(abort);
}
